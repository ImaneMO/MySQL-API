...
public class SupplierServiceImpl implements SupplierService {
	...
	public List<SupplierDto> findAll() {
		...
		for (Supplier s:suppliers) {
			suppliersDto.add(new SupplierDto(s.getId(), s.getSupplierName(), s.getService(), s.getEmail()));
		}
	...
	public SupplierDto findById(long theID) {
		...
		if(supplierWanted.isPresent()) {
			return new SupplierDto(supplierWanted.get().getId(), supplierWanted.get().getSupplierName(), supplierWanted.get().getService(), supplierWanted.get().getEmail());
		}else {
